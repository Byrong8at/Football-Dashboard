        ...
        with container1:
            try:
                ...
                for mtc in competition:
                    ...
                    if mtc not in ["Korean Super Cup", "Korea Cup","FIFA Club World Cup","AFC Champions League Elite"]:
                            def get_pts(row):
                                s = str(row['Score'])
                    ...
                    if matches != 0:
                        ...
                        with col_c:
                            st.markdown(f'<div class="metric-card"><div class="metric-label">Système</div><div class="metric-value" style="font-size:24px">{system}</div></div>', unsafe_allow_html=True)
                        with col_d:
